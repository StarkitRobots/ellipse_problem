...
from Algorithm import BullockCircleFitting, CircleModel
import threading
...
class RansacCircleHelper(object):
    ...
    def run(self)-> CircleModel:
        ...
        for trig_index in random_trigram_indices:
            ...
            try:
                temp_circle= CircleModel.GenerateModelFrom3Points(tri.P1, tri.P2, tri.P3, self.min_r, self.max_r)
            except Exception as e:
                #print("Could not generate Circle model. Error=%s" % (str(e)))
                continue
        
            inliers,goodness_score, inliners_r=self.get_inliers(temp_circle,[tri.P1,tri.P2,tri.P3])
            count_inliers=len(inliers)
            #if (len(inliners_r) > self.threshold_outlier_count):
            #print("!!!! ", len(inliners_r))
            if ((count_inliers < self.threshold_inlier_count) or (len(inliners_r) > self.threshold_outlier_count)):
    ...
    def generate_trigam_from_points(self,):
        ...
        for i in range(0,int(len(points)*0.9), 1):
            #for j in range(i+1,len(points)):
             ...
             #       lst.append(trigram)
            p0=points[i]
            j = int((len(points)-i)/4)
            p1=points[random.randint(i,i+j)]
            p2=points[random.randint(i,i+j+j)]
            trigram=TrigramOfPoints(p0,p1,p2)
            lst.append(trigram)
        return lst
        pass    
    ...
    def get_inliers(self, model: CircleModel, exclude_points:List[Point])->List[Point]:
        ...
        return (shortlist_inliners,avg_goodness, inliners_r)
        pass
    ...
    def find_model_using_gradient_descent2(self, modelhint: CircleModel, points:List[Point], lst_results):
        ...
        if (new_model == None):
            return
        new_inliers,goodness, inliers_r=self.get_inliers(new_model,[])
